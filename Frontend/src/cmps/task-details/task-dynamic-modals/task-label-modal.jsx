...
export function TaskLabelModal({ task, data, onSaveTask, setIsAddLabelModalOpen, isAddLabelModalOpen, setSelectedLabel, selectedLabel }) {
    ...
    return <>
        {!isAddLabelModalOpen && <section>
            ...
            <ul className='cmp-dynamic-options-list clean-list' >
                {toRender && toRender.map(opt =>
                    <li key={opt.id} className="cmp-dynamic-option cmp-dynamic-option-labels"
                    >
                        <input
                            onChange={(ev) => { onToggleLabel(ev, opt.id) }}
                            checked={labelIds?.includes(opt.id)}
                            className="cmp-dynamic-labels-checkbox"
                            type="checkbox"
                            id={opt.id}
                        />
                        <label htmlFor={opt.id} className="cmp-dynamic-labels-label"
                ...
                {!toRender.length && <li className="cmp-dynamic-option">No label by this name</li>}
