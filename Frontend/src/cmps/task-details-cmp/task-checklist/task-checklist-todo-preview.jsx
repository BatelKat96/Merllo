...
export function TodoPreview({ todo, updateTodo, onRemoveTodo }) {
	...
	return (
		// <Fragment>
		<label
			className="task-checklist-label"
			onClick={() => {
				onShowTodoInput(todo.id)
			}}
		>
			{/* onClick={() => { onShowTodoInput(todo.id) }} */}
			{(!isEditTodoOpen || todoId !== todo.id) && (
				<span className="task-checklist-label-span">{todo.title}</span>
			)}
			...
			{(isEditTodoOpen || todoId == todo.id) && (
				<form>
					...
					<div className="task-checklist-btn">
						<button
							className="clean-btn btn-task-details btn-checklist-save"
							type="submit"
							onClick={(ev) => onSaveTodoTitle(ev, todo.id)}
						>
							Save
						</button>
						<button
							className="clean-btn icon-task btn-checklist-cancel-container"
